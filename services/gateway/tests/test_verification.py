...
def test_parse_pytest_failures() -> None:
    ...
    assert result.failures == ["tests/test_api.py::test_create - AssertionError"]


def test_parse_jest_failures() -> None:
    output = "FAIL src/app.test.ts\n  ● app › renders"  # noqa: RUF001
    result = parse_verify_result("npm test", output, 1)
...
def test_parse_cargo_and_go_failures() -> None:
    cargo = parse_verify_result("cargo test", "test parser::rejects_invalid ... FAILED", 101)
