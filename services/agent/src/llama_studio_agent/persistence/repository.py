...
class SessionRepository:
    ...
    def save_replit_plan(self, plan: ReplitPlan) -> ReplitPlan:
        with self.db.connect() as conn:
            conn.execute(
                "INSERT INTO replit_plans(id,session_id,title,summary,status,created_at,updated_at)"
                " VALUES (?,?,?,?,?,?,?)"
        ...
        for task in plan.tasks:
            self.save_replit_task(task)
        self.touch(plan.session_id)
        return plan

    def list_replit_plans(self, session_id: UUID) -> list[ReplitPlan]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM replit_plans WHERE session_id = ? ORDER BY datetime(created_at) DESC",
                (str(session_id),),
            ).fetchall()
        plans = [self._row_to_replit_plan(r) for r in rows]
        for plan in plans:
            plan.tasks = self.list_replit_tasks(session_id, plan_id=plan.id)
        return plans

    def get_replit_plan(self, session_id: UUID, plan_id: UUID) -> ReplitPlan | None:
        with self.db.connect() as conn:
            row = conn.execute(
                "SELECT * FROM replit_plans WHERE session_id = ? AND id = ?",
                (str(session_id), str(plan_id)),
            ).fetchone()
        if row is None:
            return None
        plan = self._row_to_replit_plan(row)
        plan.tasks = self.list_replit_tasks(session_id, plan_id=plan.id)
        return plan

    def save_replit_task(self, task: ReplitTask) -> ReplitTask:
        task.updated_at = _now()
        with self.db.connect() as conn:
            conn.execute(
                "INSERT INTO replit_tasks("
                "id,session_id,plan_id,title,summary,status,priority,depends_on,files_likely_changed,"
                "done_looks_like,test_plan,workspace_path,diff,test_output,error,validation_attempts,"
                ...
                " files_likely_changed=excluded.files_likely_changed, done_looks_like=excluded.done_looks_like,"
                " test_plan=excluded.test_plan, workspace_path=excluded.workspace_path, diff=excluded.diff,"
                " test_output=excluded.test_output, error=excluded.error,"
                " validation_attempts=excluded.validation_attempts, updated_at=excluded.updated_at",
                (
                    str(task.id),
                    str(task.session_id),
                    ...
                    task.status.value,
                    task.priority.value,
                    json.dumps([str(x) for x in task.depends_on]),
                    json.dumps(task.files_likely_changed),
                    json.dumps(task.done_looks_like),
                    json.dumps(task.test_plan),
                    task.workspace_path,
                    task.diff,
                    task.test_output,
                    task.error,
                    int(task.validation_attempts or 0),
                    _iso(task.created_at),
                    _iso(task.updated_at),
                ),
            )
            conn.commit()
        self.touch(task.session_id)
        return task

    def list_replit_tasks(self, session_id: UUID, plan_id: UUID | None = None) -> list[ReplitTask]:
        with self.db.connect() as conn:
            if plan_id is None:
                rows = conn.execute(
                    "SELECT * FROM replit_tasks WHERE session_id = ? ORDER BY datetime(created_at)",
                    (str(session_id),),
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT * FROM replit_tasks WHERE session_id = ? AND plan_id = ? ORDER BY datetime(created_at)",
                    (str(session_id), str(plan_id)),
                ).fetchall()
        return [self._row_to_replit_task(r) for r in rows]

    def get_replit_task(self, session_id: UUID, task_id: UUID) -> ReplitTask | None:
        with self.db.connect() as conn:
            row = conn.execute(
                "SELECT * FROM replit_tasks WHERE session_id = ? AND id = ?",
                (str(session_id), str(task_id)),
            ).fetchone()
        return self._row_to_replit_task(row) if row else None

    def append_replit_task_log(self, log: ReplitTaskLog) -> ReplitTaskLog:
        with self.db.connect() as conn:
            conn.execute(
                "INSERT INTO replit_task_logs(id,task_id,level,message,created_at) VALUES (?,?,?,?,?)",
                (str(log.id), str(log.task_id), log.level, log.message, _iso(log.created_at)),
            )
            conn.commit()
        return log

    def list_replit_task_logs(self, task_id: UUID) -> list[ReplitTaskLog]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM replit_task_logs WHERE task_id = ? ORDER BY datetime(created_at)",
                (str(task_id),),
            ).fetchall()
        return [self._row_to_replit_log(r) for r in rows]

    def save_replit_checkpoint(self, checkpoint: ReplitCheckpoint) -> ReplitCheckpoint:
        with self.db.connect() as conn:
            conn.execute(
                "INSERT INTO replit_checkpoints(id,session_id,task_id,label,snapshot_path,files,created_at)"
                " VALUES (?,?,?,?,?,?,?)",
                (
                    str(checkpoint.id),
                    str(checkpoint.session_id),
                    str(checkpoint.task_id) if checkpoint.task_id else None,
                    checkpoint.label,
    ...
    def list_replit_checkpoints(self, session_id: UUID) -> list[ReplitCheckpoint]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM replit_checkpoints WHERE session_id = ? ORDER BY datetime(created_at) DESC",
                (str(session_id),),
            ).fetchall()
        return [self._row_to_replit_checkpoint(r) for r in rows]

    def get_replit_checkpoint(self, session_id: UUID, checkpoint_id: UUID) -> ReplitCheckpoint | None:
        with self.db.connect() as conn:
            row = conn.execute(
                "SELECT * FROM replit_checkpoints WHERE session_id = ? AND id = ?",
                (str(session_id), str(checkpoint_id)),
            ).fetchone()
        return self._row_to_replit_checkpoint(row) if row else None

    # ── helpers ─────────────────────────────────────────────────────────


    @staticmethod
    def _row_to_replit_plan(r: Any) -> ReplitPlan:
        return ReplitPlan(
            id=UUID(r["id"]),
            session_id=UUID(r["session_id"]),
            title=r["title"],
            summary=r["summary"],
            status=ReplitPlanStatus(r["status"]),
            created_at=_from_iso(r["created_at"]) or _now(),
            updated_at=_from_iso(r["updated_at"]) or _now(),
            tasks=[],
        )

    @staticmethod
    def _row_to_replit_task(r: Any) -> ReplitTask:
        try:
            attempts_raw = r["validation_attempts"]
        ...
        return ReplitTask(
            ...
            title=r["title"],
            summary=r["summary"],
            status=ReplitTaskStatus(r["status"]),
            priority=ReplitTaskPriority(r["priority"]),
            depends_on=[UUID(x) for x in json.loads(r["depends_on"] or "[]")],
            files_likely_changed=list(json.loads(r["files_likely_changed"] or "[]")),
            done_looks_like=list(json.loads(r["done_looks_like"] or "[]")),
            test_plan=list(json.loads(r["test_plan"] or "[]")),
            workspace_path=r["workspace_path"],
            diff=r["diff"],
            test_output=r["test_output"],
            error=r["error"],
            validation_attempts=int(attempts_raw or 0),
            created_at=_from_iso(r["created_at"]) or _now(),
            updated_at=_from_iso(r["updated_at"]) or _now(),
        )

    @staticmethod
    def _row_to_replit_log(r: Any) -> ReplitTaskLog:
        return ReplitTaskLog(
            id=UUID(r["id"]),
    ...
    @staticmethod
    def _row_to_replit_checkpoint(r: Any) -> ReplitCheckpoint:
        return ReplitCheckpoint(
            ...
            files=list(json.loads(r["files"] or "[]")),
            created_at=_from_iso(r["created_at"]) or _now(),
        )
