 ...
 *
 * Shows every planned step with its action badge, rationale and (for modify
 * steps) an expandable diff. Steps are checked by default and can be
 * unchecked; approving sends only the checked files as `acceptedPaths`, so the
 * gateway drops the rest from the plan before executing.
 */
  ...
  async function decide(choice: ApprovalDecision): Promise<void> {
    ...
    try {
      await onDecision({
        runId: event.runId,
        decision: choice,
        // Only send a selection when the user actually deselected something —
        // an empty list means "all steps" on the gateway side.
...
export const ROW_COMPONENTS: Record<EventType, RowComponent> = {
  ...
  "plan-ready": PlanReadyRow as RowComponent,
