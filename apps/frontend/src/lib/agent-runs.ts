/**
 * Multi-run model for tracked runs (§12.3) — zoc-agent-chat-rebuild R1.3, R2.3, task 25.1.
 *
 * Homed in `lib` because three surviving consumers read it — `features/terminal`, `lib/store.ts`,
 * and `lib/key-bindings.ts` — and it was living inside `features/agent`, which 26.1 deletes. The
 * move was free: this module has no imports at all, so nothing came with it. That is also why it
 * is an inversion rather than a shim — the dependency now runs from the dying tree into `lib`.
 *
...
export function canStartRun(runs: readonly TrackedRun[], maxConcurrentRuns: number): boolean {
...
export function upsertRun(runs: readonly TrackedRun[], next: TrackedRun): TrackedRun[] {
