/**
 * session-origin.ts — the pure foreign-session predicate and row projection (R15.7, R15.3) —
 * zoc-agent-chat-rebuild R1.3, R2.3, task 25.2.
 *
 ...
 *
 * Homed in `lib` because four surviving modules read it — `lib/store.ts`, both workspace sessions
 * surfaces, and `features/sessions/session-list-model.ts` — and it has **no imports at all**, so the
 * move is a rename. Amendment 1 names this destination exactly. It carries `sessionListItem` and
 * `rootBasename` with it because 22.5's list needs the projection and R15.10's workspace scoping needs
 * the predicate; splitting them would put one half in `lib` and leave the other to die at 26.1.
 */
