...
import type { EditorRunContext } from "@/lib/editor-context";
import type {
...
// Task 25.4 inverted this: `EditorRunContext` now lives in `lib/editor-context.ts`, the module that
// builds it, so the dependency points into `lib` rather than out of the tree 26.1 deletes. Re-exported
// rather than duplicated — this line dies with the file.
export type { EditorRunContext };
...
async function resolveBaseUrl(): Promise<string> {
  return (await resolveWorkspaceServicesEndpoint()).baseUrl;
}
...
export async function getModelBenchmarkHistory(modelId: string): Promise<ModelBenchmarkHistory> {
