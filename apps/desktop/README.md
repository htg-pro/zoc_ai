...
shell plugin, reads `ZOC_STUDIO_AGENT_PORT=<n>` from stdout, and exposes the
port to the frontend via the `agent_port` Tauri command.

## Sidecar binary

`tauri.conf.json` declares `binaries/zoc-studio-agent` as an external bin.
...
`apps/desktop/binaries/zoc-studio-agent-<triple>`. During dev the same
