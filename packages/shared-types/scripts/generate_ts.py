...
from shared_schema import agent_events, message_parts, models  # noqa: E402
...
def _has_top_level_union(ts_type: str) -> bool:
    """True when `ts_type` is a union at its outermost level.

    `"a" | "b"` is; `("a" | "b")[]` and `Record<string, A | B>` are not. Needed
    because `f"{item}[]"` around a bare union binds as `"a" | ("b"[])` — a real
    bug that produced `offeredScopes: "call" | "run" | "workspace"[]` for
    `list[PermissionScope]` before this existed.
    """
    depth = 0
    in_quote = False
    index = 0
    while index < len(ts_type):
        char = ts_type[index]
        if in_quote:
            if char == "\\":
                index += 2
...
def _as_array(item_type: str) -> str:
    return f"({item_type})[]" if _has_top_level_union(item_type) else f"{item_type}[]"
...
def generate_enum_ts(name: str, enum_class: type[Enum]) -> str:
...
def generate_interface_ts(name: str, model_class: type[BaseModel]) -> str:
    ...
    # The JSON-schema property keys are wire names (aliases where declared), but
    # `model_fields` is keyed by the Python attribute name. Every membership test
    # below is against a property key, so translate once here — otherwise every
    # aliased field with a default reads as "no default" and is emitted optional,
    # which is wrong for fields the wire always carries.
    def wire_name(field_name: str) -> str:
        info = model_fields.get(field_name)
    ...
    for field_name, field_info in model_fields.items():
        ...
        if field_info.default is not None:
            ...
            if "PydanticUndefined" not in default_str:
                fields_with_defaults.add(wire_name(field_name))
    ...
    for field_name, field_info in model_fields.items():
        ...
        if annotation and hasattr(annotation, "__origin__") and annotation.__origin__ is Literal:
            ...
            if len(literal_values) == 1:
                ...
                if has_default or has_default_factory:
                    ...
                    literal_fields_with_defaults[wire_name(field_name)] = literal_values[0]
    ...
    if hasattr(model_class, "__bases__"):
        for base in model_class.__bases__:
            if hasattr(base, "model_json_schema") and base.__name__ not in ["BaseModel", "_Base"]:
                ...
                literal_fields_with_defaults = {
                    k: v for k, v in literal_fields_with_defaults.items() if k not in base_props
                }
                break
...
#: Names in `message_parts.__all__` that collide with a name already exported by
#: `models.__all__`. Both are real, distinct concepts — `models.PermissionScope`
#: is the legacy tool-descriptor scope set (`read_fs`/`write_fs`/…) and
#: `message_parts.PermissionScope` is the grant scope a Permission_Request
#: offers (`call`/`run`/`workspace`) — so the collision is resolved by renaming
#: the newer one at the package root rather than by dropping either. The
#: original name stays reachable as `MessageParts.PermissionScope` through the
#: namespace re-export, so nothing is lost by the rename.
RENAMED_ON_REEXPORT = {"PermissionScope": "GrantScope"}


def message_parts_reexport_footer() -> list[str]:
    """Explicit, deterministic re-export list for `message-parts.ts`.

    A bare `export *` cannot be used: `PermissionScope` exists in both modules
    and TypeScript rejects an ambiguous star re-export. Computing the list from
    `__all__` keeps the footer in lockstep with the schema, so adding a part is
    still one edit in one file.
    """
    model_names = set(models.__all__)
    plain: list[str] = []
    renamed: list[str] = []
    for name in sorted(message_parts.__all__):
        if name == "MessagePartModel":
            continue
        if name in RENAMED_ON_REEXPORT:
            renamed.append(f"{name} as {RENAMED_ON_REEXPORT[name]}")
        elif name in model_names:
            # An unlisted collision is a drift bug, not something to paper over.
            raise SystemExit(
                f"message_parts.{name} collides with models.{name}; add it to "
                "RENAMED_ON_REEXPORT in generate_ts.py"
            )
        else:
            plain.append(name)
    ...
    lines.extend(f"  {n}," for n in plain + renamed)
    lines.append('} from "./message-parts";')
    lines.append("")
    return lines
...
def generate_message_parts_typescript() -> str:
    return generate_typescript(
        message_parts,
        source_name="packages/shared-types/python/shared_schema/message_parts.py",
        skip_models={"MessagePartModel"},
    )


def generated_files() -> dict[Path, str]:
    """The single registry of emitted files.

    Both `main()` and `check_drift()` read this, so registering a module is one
    ...
    return {
        ...
        ROOT / "typescript" / "src" / "message-parts.ts": generate_message_parts_typescript(),
    }
...
def main() -> None:
    ...
    if args.check:
        ...
        print(f"   Message parts: {len(message_parts.__all__)}")
